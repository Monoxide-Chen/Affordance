...
# from model.query_generator import QueryGenerationModule
...
# from point_encoder import PointNet_Encoder
# from geo_aware_pooling import GeoAwarePooling, GeoAwarePoolingBV
# from query_generator import QueryGenerationModule
...
class PointSAM(nn.Module):
    def __init__(self, normal_channel=False, local_rank=None,
        ...
        self.pos2d = nn.Parameter(torch.zeros(1, 8, self.emb_dim))
    ...
    def forward(self, text, xyz, view_mask):
        ...
        fs = self.geo_pooling(xyz, point_feature, view_mask) # [batch, V, d_model]
        ...
        query = torch.cat([fs, mean_t_feat.unsqueeze(1).repeat(1, 4, 1)], dim=1) # [batch, V+1, d_model]
        ...
        # t_feat = self.decoder(t_feat, fs.transpose(-2, -1), tgt_key_padding_mask=t_mask, query_pos=self.pos1d) # b,l,c
